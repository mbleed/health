...
<form name="frm1" method="post" action="pi_qa.php">
<div id=step>
...
eoq;
	if ($_POST["submit"] <> "") {
		...
		if ($rows = $db->get_results($sql)) {
			$body .= "<ul>";
			foreach($rows as $row) {
				$body .= "<li><a href=pi_qa.php?que=$row->que_id>$row->que_txt</a></li>";
			}
			echo "</ul>";
...
eoq;
    	foreach ($question_types as $qt) {
     	if ($expand_to_que_type == $qt->que_type_id) $expand = 'true'; else $expand = 'false';
		$body .= "var node_$qt->que_type_id = new YAHOO.widget.TextNode(\"$qt->que_type\", root, $expand, false); \n";
		...
		foreach ($questions as $q) {
	   		$body .= "var q_$q->que_id = new YAHOO.widget.HTMLNode(\"<a href='pi_qa.php?que=$q->que_id'>$q->que_txt</a>\", node_$qt->que_type_id, false); \n";
			//link = pi_qa.php?que=$q->que_id
		}
	}
	$body .= <<<eoq

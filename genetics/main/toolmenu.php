...
var AjaxNote = {
	handleSuccess:function(o){ document.note_form.save_txt.value = ''; document.getElementById('pg_notes').innerHTML = o.responseText; },
	handleFailure:function(o){ alert('Error, try your request again.'); },
	startRequest:function() {
		var oForm = document.getElementById('note_form');
		YAHOO.util.Connect.setForm(oForm);
	   	YAHOO.util.Connect.asyncRequest('POST', 'save_notes.php', AjaxNoteCallback);
	}
};
var AjaxNoteCallback =
{
	success:AjaxNote.handleSuccess,
	failure:AjaxNote.handleFailure,
	scope: AjaxNote
};
function clickSave() {
	AjaxNote.startRequest();
}

var AjaxMsg = {
	handleSuccess:function(o){ document.msg_form.msg_txt.value = ''; document.getElementById('pg_msgs').innerHTML = o.responseText; },
	handleFailure:function(o){ alert('Error, try your request again.'); },
	startRequest:function() {
		var oForm = document.getElementById('msg_form');
		YAHOO.util.Connect.setForm(oForm);
	   	YAHOO.util.Connect.asyncRequest('POST', 'save_msgs.php', AjaxMsgCallback);
	}
};
var AjaxMsgCallback =
{
	success:AjaxMsg.handleSuccess,
	failure:AjaxMsg.handleFailure,
	scope: AjaxMsg
};
function clickMsgSave() {
	AjaxMsg.startRequest();
}
</script>
...
$toolmenu .= <<<eoq
	...
	<form name="note_form" id="note_form" method="post">
		<textarea name=save_txt rows=3 class="input-medium"></textarea>
		<br /> 
...
#pg_msgs p {
	...
	<form name="msg_form" id="msg_form" method="post">
		<textarea name=msg_txt class="input-medium" rows=3></textarea>
...
//*************************   Page Resources ******************************
	if ($rows = $db->get_results("SELECT * FROM page_resources WHERE pr_pg = '$pg'")) {
	  	$toolmenu .= "<div class='well'>";
	  	$toolmenu .= "<h1>Resources</h1>";
	  	$toolmenu .= "<ul>";
    		foreach ($rows as $row) {
...
//*************************   Questions  ******************************
	...
	$toolmenu .= "<div class='well'>";
	$toolmenu .= "<h1>Questions</h1>";
	$toolmenu .= <<<eoq

	<form name=qu method=post action="nt_qu_add.php">
	Enter a question you need to answer:<br>
	<textarea name="qu" rows="3" class="input-medium" class="mceNoEditor"></textarea>
	<br>

 ...
 *
 * $Header: /cvs/bbclone/show_regression.php,v 1.8 2007/01/27 22:01:13 christoph Exp $
 *
...
function print_summary() {
  ...
  global $all, $browser_ok, $browser_failed, $browser_todo, $os_ok, $os_failed, $os_todo, $robot_ok, $robot_failed, $robot_todo;
...
<form action="'.((_BBC_PHP < 410) ? $_PHP_SELF : $_SERVER['PHP_SELF']).'" method="post">
...
$hide_ok = (isset($get['hide_ok']) && ($get['hide_ok'] == 'true'));
    ...
    while (list(${$i."_name"}, ${$i."_elem"}) = each($$i)) {
      ...
      while (list($pattern, $note) = each(${$i."_elem"}['rule'])) {
        ...
        if (($i == "browser") && (strcmp($expected_browser_name, ${$i."_name"}) != 0)) {
          ...
          if (!$hide_ok) echo add_row($user_agent, 'green', 'OK', NULL, NULL);
          ${$i.'_ok'}++;
        }
        flush();
